...
class CollisionWorld(robot.Item):
    ...
    def __init__(self, name, node):
        ...
        self._object_pub = node._conn.create_publisher(CollisionObject, self._setting['control_topic'], 100)
    ...
    def _wait_object_id_used(self, id, timeout=1.0):
        ...
        while not rclpy.ok():
            if self._is_object_id_used(id):
                return True
            elif self.get_clock().now() - start > rclpy.duration.Duration(seconds=timeout):
                return False
            rate.sleep()
