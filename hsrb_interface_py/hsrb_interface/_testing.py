...
class RosMockTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch("control_msgs.action._follow_joint_trajectory.FollowJointTrajectory_GetResult_Response")
        ...
        patcher = patch("hsrb_interface.utils.get_parameters_from_another_node")
...
class HsrbInterfaceTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            static_distance_map_sub = self.robot._conn.create_subscription(OccupancyGrid,'/static_distance_map',self.map_message_callback,qos_profile=qos_profile)
            self.wait_for_message(self.robot._conn,static_distance_map_sub)   
            static_obstacle_map_sub = self.robot._conn.create_subscription(OccupancyGrid,'/static_obstacle_map',self.map_message_callback,qos_profile=qos_profile)
            self.wait_for_message(self.robot._conn,static_obstacle_map_sub)
            dynamic_obstacle_map_sub = self.robot._conn.create_subscription(OccupancyGrid,'/dynamic_obstacle_map',self.map_message_callback,qos_profile=qos_profile)
            self.wait_for_message(self.robot._conn,dynamic_obstacle_map_sub)
        except Exception:
        ...
        self.tf_buffer = tf2_ros.Buffer(cache_time=rclpy.duration.Duration(seconds=10),node=self.robot._conn)
        self.tf_listner = tf2_ros.TransformListener(self.tf_buffer, self.robot._conn)
