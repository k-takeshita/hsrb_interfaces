...
class CollisionWorldTest(testing.HsrbInterfaceTest):
    ...
    def setUp(self):
        ...
        """
        node = Node("collisionWorld_test_node")
        
        known_object_pub = node.create_publisher(CollisionObject,'known_object',1)
        start = node..get_clock().now()
        while known_object_pub.get_subscription_count() == 0:
